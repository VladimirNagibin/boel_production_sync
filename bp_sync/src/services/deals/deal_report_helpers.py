...
EXPORT_COLUMNS_ORDER = (
    ...
    "Дата лида",
    "Название лида",
    "Ответственный по лиду",
    "Создатель лида",
    "Источник лида",
    "Тип лида",
    "Стадия лида",
    "ИД сайта Calltouch лид",
    "ИД клиента Яндекс лид",
    # Поля счета
    "ИД счета",
    "Дата счета",
    "Название счета",
    "Ответственный по счету",
    "Номер счета",
    "Компания",
    "Выгружен в 1С",
    "Сумма счета",
...
async def _build_data_row(deal: DealDB) -> dict[str, Any]:
    ...
    try:
        ...
        # else:
        #    ...
        ordered_row = {key: row.get(key, "") for key in EXPORT_COLUMNS_ORDER}
