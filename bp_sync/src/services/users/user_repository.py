...
class UserRepository(BaseRepository[UserDB, UserCreate, UserUpdate, int]):
        ...
        try:
            query = (
                ...
                .options(
                    ...
                    selectinload(ManagerDB.default_company),
                )
            )
    ...
    async def get_default_company_manager(self, user_id: int) -> int | None:
        """Получить компанию по умолчанию для менеджера"""
        try:
            manager = await self.get_manager(user_id)
            if manager:
                company_id = manager.default_company_id
                return company_id  # type: ignore[no-any-return]
            return None
        except RuntimeError as e:
            logger.error(
                f"Error getting default company for manager {user_id}: {e}"
            )
            return None
